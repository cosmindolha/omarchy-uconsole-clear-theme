...
After installing Voxtype, run `./scripts/install-faster-whisper.sh`. This installs a checksum-verified ARM64 uv runtime, isolated Python 3.12 environment, pinned faster-whisper/CTranslate2 dependencies, and the pinned multilingual base model. It keeps the model loaded on the CPU using INT8 and four threads. Voxtype still handles the microphone, gamepad shortcut and typing, but sends transcription requests only to `127.0.0.1:8769`. Audio stays on the uConsole; transcription uses local model files; internet is used only to download models you select. The previous Voxtype configuration is saved as `config.toml.before-faster-whisper`.

**Menu → Setup → Dictation** opens the screen below, with compact language buttons and a model selector. **1/E** saves English; **2/R** saves Romanian. Arrow keys and Enter also work. English is the initial default. Language changes persist immediately; finish an active dictation before switching. **M** focuses the model list, arrow keys select, and **S** downloads/uses the chosen model. The list comes from the installed faster-whisper version (19 model names/aliases in 1.2.1), marks installed models, and includes tiny/base/small/medium/large, English-only and distilled variants. English-only models are unavailable for Romanian. Downloads and model loading run in the background; the old model remains active until the replacement loads and its selection is saved. Failed switches keep the working model and show the error. Downloaded models remain available offline and the selection survives login/restart. Large models can take minutes on CM5; Voxtype allows up to ten minutes for a transcription. Base remains the initial default. Language detection is disabled. The backend reads `~/.config/uconsole/dictation.json` for the selected language and model, overriding Voxtype's cached language field. This also preserves Romanian as Romanian rather than translating it to English.
